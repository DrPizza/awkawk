...
player_controls::player_controls(Player* player_, window* parent_, IDirect3DDevice9Ptr device_) : message_handler(parent_), device(device_),
                                                                                                  ui_reveal_percentage(0.0f), text_size(14.0f),
                                                                                                  caption_text(L"<no file loaded>"), chosen_image(normal),
                                                                                                  dragging_position_tracker(false), position_drag_percentage(0.0f),
                                                                                                  ...
                                                                                                  player(player_)
{
	...
	controls.reset(new strip(device, 8));
	trackbar.reset(new strip(device, 8));
	position_tracker.reset(new strip(device, 4));
	shadowed_position_tracker.reset(new strip(device, 4));
	volume_tracker.reset(new strip(device, 4));
	caption.reset(new strip(device, 8));
	initialize();
}

void player_controls::initialize()
{
...
bool player_controls::handles_message(HWND wnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		{
			...
			::OffsetRect(&caption_rect, window_rect.left, window_rect.top);
			...
			::OffsetRect(&controls_rect, window_rect.left, window_rect.top);
			bool handled(::PtInRect(&caption_rect, pt) == TRUE || ::PtInRect(&controls_rect, pt) == TRUE);
			//dout << std::dec << std::boolalpha
			//     << "caption: (" << caption_rect.left << ", " << caption_rect.top << ") (" << caption_rect.right << ", " << caption_rect.bottom << ") "
			//     << "controls: (" << controls_rect.left << ", " << controls_rect.top << ") (" << controls_rect.right << ", " << controls_rect.bottom << ") "
			//     << "cursor (" << pt.x << ", " << pt.y << ") "
...
LRESULT CALLBACK player_controls::message_proc(HWND wnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
...
void player_controls::onLeftButtonDown(HWND wnd, BOOL doubleClick, int x, int y, UINT keyFlags)
{
	RECT window_rect(parent->get_window_rect());
	LONG width(window_rect.right - window_rect.left);
	LONG height(window_rect.bottom - window_rect.top);

	static const int sysMenuSize(16);
	static const int minSize(9);
	static const int maxSize(9);
	static const int closeSize(10);

	// SYS        <title>        min max close
	// 16                         9   9   10

	if(0 <= x && x <= sysMenuSize && 0 <= y && y <= sysMenuSize)
	{
		::TrackPopupMenu(::GetSystemMenu(wnd, FALSE), TPM_LEFTBUTTON | TPM_RIGHTBUTTON, window_rect.left + x, window_rect.top + y, 0, wnd, NULL);
		return;
	}
	else if(width - (minSize + maxSize + closeSize) <= x && x <= width - (maxSize + closeSize) && 0 <= y && y <= sysMenuSize)
	{
		parent->show_window(SW_MINIMIZE);
		return;
	}
	else if(width - (maxSize + closeSize) <= x && x <= width - closeSize && 0 <= y && y <= sysMenuSize)
	{
		player->toggle_fullscreen();
	}
	else if(width - closeSize <= x && x <= width && 0 <= y && y <= sysMenuSize)
	{
	...
	else if(control_images chosen = hit_test_controls(x, y))
	{
		...
		case unknown2:
			break;
		case open:
			parent->post_message(WM_COMMAND, IDM_LOAD_FILE, 0);
			return;
		}
	}
	else if(hit_test_position_slider(x, y))
	{
	...
	FORWARD_WM_LBUTTONDOWN(wnd, doubleClick, x, y, keyFlags, parent->message_proc);
}

void player_controls::onLeftButtonUp(HWND wnd, int x, int y, UINT keyFlags)
{
...
player_controls::control_images player_controls::hit_test_controls(int x, int y)
{
	...
	::OffsetRect(&controls_rect, 0, -controls_rect.top);
	//dout << std::dec << std::boolalpha
	//     << "controls: (" << controls_rect.left << ", " << controls_rect.top << ") (" << controls_rect.right << ", " << controls_rect.bottom << ") "
	//     << "cursor (" << pt.x << ", " << pt.y << ") " << std::endl;
	for(size_t i(0); i < sizeof(skin.button_definitions) / sizeof(skin.button_definitions[0]); ++i)
	{
		if(point_in_circle(pt, skin.button_definitions[i].centre, skin.button_definitions[i].radius))
		{
			return skin.button_definitions[i].value;
		}
	}
	return normal;
}
...
void player_controls::end_device_loss(IDirect3DDevice9Ptr device_)
{
	device = device_;
	initialize();
	controls->end_device_loss(device);
	trackbar->end_device_loss(device);
	position_tracker->end_device_loss(device);
	shadowed_position_tracker->end_device_loss(device);
	volume_tracker->end_device_loss(device);
	caption->end_device_loss(device);
}
...
void player_controls::calculate_caption()
{
	...
	HFONT font(::CreateFont(static_cast<int>(text_size), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_TT_ONLY_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, L"Verdana"));
	ON_BLOCK_EXIT(&::DeleteObject, font);
	::SelectObject(dc, font);
	ID3DXMeshPtr mesh;
	FAIL_THROW(D3DXCreateText(device, dc, compact_caption_text.c_str(), 0.001f, 0.0f, &mesh, NULL, NULL));
...
void player_controls::set_compact_filename()
{
	...
	SIZE window_size(player->get_window_dimensions());
	::PathCompactPath(dc, buffer.get(), window_size.cx - static_cast<UINT>(skin.caption.system_menu_width + skin.caption.system_title_padding + skin.caption.title_window_padding + skin.caption.window_control_width));
...
void player_controls::begin_device_loss()
{
	device = NULL;
	...
	trackbar->begin_device_loss();
	position_tracker->begin_device_loss();
	shadowed_position_tracker->begin_device_loss();
	volume_tracker->begin_device_loss();
	caption->begin_device_loss();
}
