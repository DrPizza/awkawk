...
struct player_window : window, boost::noncopyable
{
	...
	void set_aspect_ratio(rational_type ar)
	{
		aspect_ratio = ar;
	}

	rational_type get_aspect_ratio() const
	{
		return aspect_ratio;
	}
	...
	void onContextMenu(HWND wnd, HWND hwndContext, INT x, INT y);
	...
	void async(std::function<void(player_window*)> f) {
		thread_queue.async([=] {
			f(this);
		});
	}
...
protected:
	virtual bool prefilter_message(MSG* msg)
	{
		...
		return window::prefilter_message(msg);
	}
...
private:
	...
	rational_type aspect_ratio;

	gcd::queue thread_queue;
};
