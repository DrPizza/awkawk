...
{
	typedef DWORD(T::* F)(P);
	std::unique_ptr<utility::ThreadInfoMemberFn<T, P, F> > ti(new utility::ThreadInfoMemberFn<T, P, F>(obj, func, data, nullptr));
...
#endif
	{
		std::unique_ptr<utility::ThreadInfoNonMemberFn<P, F> > ti(static_cast<utility::ThreadInfoNonMemberFn<P, F>*>(data));
		if(ti->name != nullptr)
		{
...
{
	typedef DWORD(*F)(P);
	std::unique_ptr<utility::ThreadInfoNonMemberFn<P, F> > ti(new utility::ThreadInfoNonMemberFn<P, F>(func, data, nullptr));
...
{
	typedef DWORD(*F)(P);
	std::unique_ptr<utility::ThreadInfoNonMemberFn<P, F> > ti(new utility::ThreadInfoNonMemberFn<P, F>(func, data, name));
	HANDLE rv(::CreateThread(sa, stackSize, utility::ThreadDispatchNonMemberFn<P, F>, static_cast<void*>(ti.get()), flags, tid));
	if(NULL != rv) { ti.release(); }
	return rv;
}
