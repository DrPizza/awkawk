...
{
	window(const wchar_t* window_class_name, UINT style, HICON icon, HCURSOR cursor, HBRUSH background, const wchar_t* menu_name, const wchar_t* accelerators_id) : control(nullptr),
...
{
	dialogue() : control(nullptr), instance(::GetModuleHandle(NULL))
	{
	}
	...
	{
		::InvalidateRect(get_window(), nullptr, TRUE);
	}
