...
struct menu : control, menu_entry
{
	...
	{
		items.insert(items.begin() + position, boost::shared_ptr<menu_item>(new_item));
		MENUITEMINFOW mii(new_item->get_item_info());
...
protected:
	...
	const std::vector<boost::shared_ptr<menu_item> > get_items() const
	{
		return items;
	}
...
struct edit_box_item : control, virtual menu_entry
{
	edit_box_item(window* owner, menu* parent, UINT id, const std::wstring& caption) : control(owner),
	                                                                                   ...
	                                                                                   menu_item(parent, id),
	                                                                                   created(::CreateEventW(NULL, FALSE, FALSE, NULL))
	{
	}
	...
	virtual void onPostInitMenuPopup(HWND wnd, HMENU menu, UINT item, BOOL window_menu)
	{
		...
		thread = utility::CreateThread(NULL, 0, this, &edit_box_item::show_box, reinterpret_cast<void*>(0), 0, &thread_id);
...
private:
	...
	DWORD show_box(void*)
	{
		edit_control = ::CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_TOPMOST, L"EDIT", L"Lorem Ipsum", WS_POPUP | WS_BORDER, edit_rect.left, edit_rect.top, RW(&edit_rect), RH(&edit_rect), NULL, NULL, ::GetModuleHandle(NULL), NULL);
