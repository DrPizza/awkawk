...
void themed_separator_item::onDrawItem(HWND window, const DRAWITEMSTRUCT* draw_item)
{
	...
	::DrawThemeBackground(get_metrics().theme, draw_item->hDC, MENU_POPUPSEPARATOR, 0, &dim.parts[themed_item::menu_separator], NULL);
}
...
void themed_string_item::onMeasureItem(HWND window, MEASUREITEMSTRUCT* measure_item)
{
	...
	RECT rcText = { 0 };
	::GetThemeTextExtent(get_metrics().theme, dc, MENU_POPUPITEM, 0, get_caption().c_str(), static_cast<int>(get_caption().size()), DT_LEFT | DT_SINGLELINE | DT_EXPANDTABS, NULL, &rcText);
...
void themed_string_item::onDrawItem(HWND window, const DRAWITEMSTRUCT* draw_item)
{
	...
	if(::IsAppThemed() != TRUE)
	{
		...
		if(mii.fState & MFS_CHECKED)
		{
			...
			HBITMAP checkbox(::CreateBitmap(width, height, 1, 1, NULL));
		...
		else
		{
			...
			::DrawStateW(draw_item->hDC, ::GetSysColorBrush(COLOR_GRAYTEXT), NULL, reinterpret_cast<LPARAM>(caption_left.c_str()), static_cast<WPARAM>(caption_left.size()), r.left, r.top, 0, 0, DST_PREFIXTEXT | action | accel);
			if(os != std::wstring::npos)
			{
				::DrawStateW(draw_item->hDC, ::GetSysColorBrush(COLOR_GRAYTEXT), NULL, reinterpret_cast<LPARAM>(caption_right.c_str()), static_cast<WPARAM>(caption_right.size()), r.left, r.top, 0, 0, DST_PREFIXTEXT | DSS_RIGHT | action | accel);
			}
		}
		return;
	}
