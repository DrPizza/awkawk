...
LRESULT CALLBACK load_url_dialogue::message_proc(HWND wnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
	case WM_COMMAND:
		...
		case IDOK:
			//::SendDlgItemMessage(wnd, IDC_URL, WM_GETTEXT, 
			{
				...
				boost::scoped_array<wchar_t> buffer(new wchar_t[message_length]);
