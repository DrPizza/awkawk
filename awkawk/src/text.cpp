...
// based on http://graphicsbb.itgo.com/solutions/extrude.html
...
struct PathPolygon : public std::vector<contour>
{
	PathPolygon(HDC dc)
	{
		...
		const int num_points(::GetPath(dc, nullptr, nullptr, 0));
		std::unique_ptr<POINT[]> points(new POINT[num_points]);
		std::unique_ptr<BYTE[]> types(new BYTE[num_points]);
...
{
	if(mesh == nullptr || device == nullptr || text == nullptr)
	{
		return E_POINTER;
	}
	...
	std::unique_ptr<D3DXVECTOR4[]> transformed_vertices(new D3DXVECTOR4[msh->GetNumVertices()]);
	D3DXMATRIX scaling;
	...
	FAIL_RET(msh->OptimizeInplace(D3DXMESHOPT_COMPACT | D3DXMESHOPT_ATTRSORT | D3DXMESHOPT_VERTEXCACHE, static_cast<const DWORD*>(buffer->GetBufferPointer()), nullptr, nullptr, nullptr));
