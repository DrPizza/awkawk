...
LONG WINAPI exception_filter(EXCEPTION_POINTERS* exception_pointers)
{
	...
	std::unique_ptr<wchar_t[]> temp_path(new wchar_t[_MAX_PATH + 1]);
...
void show_locks()
{
	for(std::map<DWORD, std::shared_ptr<utility::lock_tracker::lock_info> >::const_iterator it(utility::tracker.info.begin()), end(utility::tracker.info.end()); it != end; ++it)
	{
		std::shared_ptr<utility::lock_tracker::lock_info> info(it->second);
		...
		for(std::list<utility::lock_tracker::lock_manipulation>::const_iterator lit(info->current_sequence.begin()), lend(info->current_sequence.end()); lit != lend; ++lit)
		{
			...
			dout << " " << lit->section->name << " (" << std::hex << lit->section << ") ";
