...
player_window::player_window(awkawk* player_) : window(L"awkawk class", CS_DBLCLKS, ::LoadIconW(::GetModuleHandleW(NULL), MAKEINTRESOURCEW(IDI_PLAYER)), ::LoadCursorW(NULL, MAKEINTRESOURCEW(IDC_ARROW)), static_cast<HBRUSH>(::GetStockObject(BLACK_BRUSH)), nullptr, MAKEINTRESOURCEW(IDR_ACCELERATORS)),
                                                player(player_),
                                                tracking(false),
                                                dragging(false),
                                                menu_visible(false),
...
{
	std::unique_ptr<wchar_t[]> buffer(new wchar_t[256]);
...
{
	OPENFILENAMEW ofn = {0};
	std::unique_ptr<wchar_t[]> buffer(new wchar_t[0xffff]);
	buffer[0] = L'\0';
...
void player_window::onContextMenu(HWND, HWND, INT x, INT y)
{
	main_menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON | TPM_RECURSE, x, y, 0, get_window(), nullptr);
	return;
}
...
BOOL player_window::onSizing(HWND, UINT edge, RECT* coords)
{
	...
	case WMSZ_RIGHT:
		...
		sz = fix_ar_height(sz, get_aspect_ratio());
		coords->bottom = coords->top + sz.cy;
		break;
	case WMSZ_TOP:
	case WMSZ_BOTTOM:
		// if it's being made taller, preserve the height, adjust the width
		sz = fix_ar_width(sz, get_aspect_ratio());
		coords->right = coords->left + sz.cx;
		break;
	// if the attempted AR > actual AR, preserve the width, adjust the height
	// else, preserve the height, adjust the width
	case WMSZ_TOPLEFT:
		if(ar > get_aspect_ratio())
		{
			sz = fix_ar_height(sz, get_aspect_ratio());
			coords->top = coords->bottom - sz.cy;
		}
		else
		{
			sz = fix_ar_width(sz, get_aspect_ratio());
			coords->left = coords->right - sz.cx;
		}
		break;
	case WMSZ_TOPRIGHT:
		if(ar > get_aspect_ratio())
...
void player_window::onSize(HWND, UINT size_type, int, int)
{
	...
	{
	case SIZE_MAXIMIZED:
		::SetTimer(get_window(), mouse_kill, 1000, nullptr);
		break;
	case SIZE_MAXHIDE:
	case SIZE_MAXSHOW:
	case SIZE_MINIMIZED:
...
void player_window::onGetMinMaxInfo(HWND, MINMAXINFO* min_max_info)
{
	...
	SIZE working_size = { working_width, working_height };
	...
	if(!player->is_fullscreen())
	{
		rational_type ar(working_width, working_height);

		if(ar > get_aspect_ratio()) // preserve height
		{
			working_size = fix_ar_width(working_size, get_aspect_ratio());
		}
		else // preserve width
		{
			working_size = fix_ar_height(working_size, get_aspect_ratio());
		}
	}
	...
	min_max_info->ptMinTrackSize.y = static_cast<LONG>((static_cast<rational_type::int_type>(min_max_info->ptMinTrackSize.x) * get_aspect_ratio().denominator()) / get_aspect_ratio().numerator());
...
{
	FORMATETC fmt = { CF_HDROP, nullptr, DVASPECT_ICON, -1, TYMED_HGLOBAL};
	...
	const UINT file_count(::DragQueryFileW(reinterpret_cast<HDROP>(medium.hGlobal), 0xffffffff, nullptr, 0));
	if(1 != file_count)
	{
		return L"";
	}
	const UINT buffer_size(::DragQueryFileW(reinterpret_cast<HDROP>(medium.hGlobal), 0, nullptr, 0));
